...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        // ===============================================================
        // MODIFICATION FOR EXCLUSIVE CACHE (Clean Victim Eviction)
        // ===============================================================
        // For exclusive caches, we also send clean victims down to the next level.
        else if (block[set][way].valid && lower_level)
        {
            // Check if the lower level WQ has room. We use the WQ for simplicity.
            if (lower_level->get_occupancy(2, block[set][way].address) < lower_level->get_size(2, block[set][way].address))
            {
                PACKET victim_packet;

                victim_packet.fill_level = fill_level << 1;
                victim_packet.cpu = fill_cpu;
                victim_packet.address = block[set][way].address;
                victim_packet.full_addr = block[set][way].full_addr;
                victim_packet.data = block[set][way].data;
                victim_packet.type = WRITEBACK; // Treat it as a writeback, but it's not dirty.
                victim_packet.event_cycle = current_core_cycle[fill_cpu];

                lower_level->add_wq(&victim_packet);
            }
            else
            {
                // lower level WQ is full, stall the fill. This behavior is consistent
                // with how dirty victims are handled.
                do_fill = 0;
                lower_level->increment_WQ_FULL(block[set][way].address);
                STALL[MSHR.entry[mshr_index].type]++;
            }
        ...
        {
            // ===============================================================
            // MODIFICATION FOR EXCLUSIVE CACHE (Conditional Fill)
            // ===============================================================
            // An exclusive cache only fills a line if the request was for itself.
            // If fill_level < this->fill_level, it means the request originated
            // from an upper cache, so we should just forward it without filling.
            if (MSHR.entry[mshr_index].fill_level >= fill_level)
            {
                ...
                // ===============================================================
            } // End of the new "if" condition for exclusive fill
            // ===============================================================
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            { // read hit
                ...
                if (RQ.entry[index].fill_level < fill_level)
                {
                    ...
                    // ===============================================================
                    // MODIFICATION FOR EXCLUSIVE CACHE (Invalidate on Hit)
                    // ===============================================================
                    // If this is L2 or LLC and we are sending the data up to a higher
                    // cache level, we must invalidate our own copy to enforce exclusivity.
                    if (cache_type == IS_L2C || cache_type == IS_LLC)
                    {
                        invalidate_entry(block[set][way].tag);
                    }
                    // ===============================================================
                }
